...
class BirthdayWisher {
  ...
  private async birthdayQuestions(names: string[]) {
    return await inquirer.prompt([
      ...
      },
      {
        ...
        name : 'customWish',
        type : 'input',
        when : (birthdayAnswers: any) =>
            birthdayAnswers.wish.indexOf('custom message') > -1,
      }
    ]);
  }
  /**
   ...
   */
  public async wishAll(birthdayNames: string[], birthdayTexts: ElementHandle[],
                       wish: string) {
    try {
      await this.page.enterAllElementText(birthdayTexts, wish);
      console.log(chalk.green('\nSuccessfully wished:\n'));
      birthdayNames.forEach(
          (name: string) => { console.log(chalk.green(name + '\n')); });
    } catch (Exception) {
      throw new Error(Exception.toString());
    }
  }
    ...
    try {
      if (await this.page.isElementExists(locators.birthdayTodayCard)) {
        ...
        if (!config.firstLogin) {
          if (config.day === today) {
            if (config.birthdayNames.length > 0) {
              await this.birthdayWish(credentials, config.birthdayNames,
                                      birthdayTexts);
        ...
        console.error(
            '\n' +
            chalk.red(
                'Uh oh, looks like none of your friends have birthdays today!\n'));
      }
   ...
   * @param  {ElementHandle[]} birthdayTexts
   */
  public async birthdayWish(credentials: any, birthdayNames: string[],
                            birthdayTexts: ElementHandle[]) {
    try {
      ...
      if (birthdayAnswers.wish.indexOf('custom message') > 0) {
        await this.page.enterElementText(indexOfBirthdayName, birthdayTexts,
                                         birthdayAnswers.customWish);

      } else {
        await this.page.enterElementText(indexOfBirthdayName, birthdayTexts,
                                         birthdayAnswers.wish);
      }
      console.log(
